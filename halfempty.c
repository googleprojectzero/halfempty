...
static const GOptionEntry kThreadOptions[] = {
    { "num-threads", 'P', G_OPTION_FLAG_NONE, G_OPTION_ARG_INT,
        &kProcessThreads,
        "How many threads to use (default=ncores+1).",
        "threads" },
    { "cleanup-threads", 0, G_OPTION_FLAG_NONE, G_OPTION_ARG_INT,
        &kCleanupThreads,
        "Number of threads used to garbage collect (default=4).",
        "threads" },
    { "max-queue", 0, G_OPTION_FLAG_NONE, G_OPTION_ARG_INT,
        &kMaxUnprocessed,
        "Maximum number of unprocessed workunits (default=4).",
        "N" },
    { "poll-delay", 0, G_OPTION_FLAG_NONE, G_OPTION_ARG_INT,
        &kWorkerPollDelay,
        "How long to sleep between checking queue status (default=10000).",
        "usecs" },
    { NULL },
};

static const GOptionEntry kDebugOptions[] = {
    { "generate-dot", 0, G_OPTION_FLAG_NONE, G_OPTION_ARG_NONE,
        &kGenerateDotFile,
        "Generate a DOT file to display the tree status (default=off).",
        NULL },
    { "collapse", 0, G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_NONE,
        &kSimplifyDotFile,
        "Collapse branches in generated dot files (default=off).",
        NULL },
    { "verbosity", 'v', G_OPTION_FLAG_NONE, G_OPTION_ARG_INT,
        &kVerbosity,
        "Set verbosity level (default=0).",
        "level" },
    { "sleep", 0, G_OPTION_FLAG_HIDDEN, G_OPTION_ARG_INT,
        &kSleepSeconds,
        "Sleep this many seconds before execve() (default=0).",
        "seconds" },
    { NULL },
};
...
static const GOptionEntry kProcessOptions[] = {
    ...
    { "limit", 0, G_OPTION_FLAG_NONE, G_OPTION_ARG_CALLBACK,
        ...
        "RLIMIT_RESOURCE=N" },
    { "inherit-stdout", 0, G_OPTION_FLAG_REVERSE, G_OPTION_ARG_NONE,
        &kSilenceChildStdout,
        "Don't redirect child stdout to /dev/null (default=redirect)",
        NULL },
    { "inherit-stderr", 0, G_OPTION_FLAG_REVERSE, G_OPTION_ARG_NONE,
        &kSilenceChildStderr,
...
static const GOptionEntry kStandardOptions[] = {
    ...
    { "stable", 0, G_OPTION_FLAG_NONE, G_OPTION_ARG_NONE,
        ...
        NULL },
    { "quiet", 'q', G_OPTION_FLAG_NONE, G_OPTION_ARG_NONE, &kQuiet,
        "Minimize all messages, only print errors (default=false).",
        NULL },
    { "continue", 0, G_OPTION_FLAG_NONE, G_OPTION_ARG_NONE, &kContinueSearch,
        "Don't exit, keep trying until interrupted (default=false).",
        NULL },
    { "noverify", 0, G_OPTION_FLAG_REVERSE, G_OPTION_ARG_NONE, &kVerifyInput,
...
int main(int argc, char **argv)
{
    ...
    for (gint k = 0; k < kNumStrategies; k++) {
        GOptionGroup *stratopts;

        stratopts  = g_option_group_new(kStrategyList[k].name,
        ...
        g_info("Strategy %u: \"%s\", %s",
               k + 1,
               kStrategyList[k].name,
               kStrategyList[k].description);
    }

    // Parse all options.
    if (g_option_context_parse(context, &argc, &argv, NULL) == false) {
        g_message("parsing commandline options failed, see --help for information");
        return EXIT_FAILURE;
    }
    ...
    if (argc != 3) {
        g_message("You must specify two parameters, a test program and an inputfile");
        return EXIT_FAILURE;
    }

    // First parameter is the script.
    if (g_access(kCommandPath = argv[1], X_OK) != 0) {
        g_message("The test program `%s` does not seem to be executable.",
                  kCommandPath);
        return EXIT_FAILURE;
    }
    ...
    if (g_access(kInputFile = argv[2], R_OK) != 0) {
        g_message("The inputfile `%s` does not seem to be readable.",
                  kInputFile);
        return EXIT_FAILURE;
    }
